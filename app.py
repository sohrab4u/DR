...
with st.sidebar:
    ...
    selected_range = st.date_input("Month Range", value=(min_date, max_date))
...
monthly_df = filtered_df.groupby(["Month Date", "Referral Category"], as_index=False)["Count"].sum().sort_values("Month Date")
facility_df = filtered_df.groupby("Facility", as_index=False)["Count"].sum().sort_values(["Count", "Facility"], ascending=[False, True])
...
with row1_col1:
    ...
    fig1.update_layout(template="plotly_white", xaxis_title="Month", yaxis_title="Count", legend_title="Category", height=420)
...
with row1_col2:
    ...
    fig2.update_layout(template="plotly_white", yaxis={"categoryorder": "total ascending"}, height=420)
...
with row2_col2:
    ...
    fig4.update_layout(template="plotly_white", xaxis_tickangle=-30, height=420)
